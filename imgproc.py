...
def image2tensor(image: np.ndarray, range_norm: bool, half: bool) -> paddle.Tensor:
...
def tensor2image(tensor: paddle.Tensor, range_norm: bool, half: bool) -> Any:
    ...
    image = np.transpose(image, [1, 2, 0])  # HxWxC
    image = np.clip(image, 0., 1.)  # CxHxW
...
def cubic(x: Any):
    ...
    absx2 = absx ** 2
    absx3 = absx ** 3
    return (1.5 * absx3 - 2.5 * absx2 + 1) * ((absx <= 1)) + (-0.5 * absx3 + 2.5 * absx2 - 4 * absx + 2) * (
        ((absx > 1) * (absx <= 2)))
...
def calculate_weights_indices(in_length: int, out_length: int, scale: float, kernel_width: int, antialiasing: bool):
    ...
    ''''
    indices = left.view(out_length, 1).expand(out_length, p) + paddle.linspace(0, p - 1, p).view(1, p).expand(
        out_length, p)'''
    indices = paddle.expand(paddle.reshape(left,[out_length, 1]),shape=[out_length, p])+paddle.expand(paddle.reshape(
       paddle.linspace(0,p-1,p),[1,p] ),shape=[out_length, p])
    # The weights used to compute the k-th output pixel are in row k of the
    # weights matrix.
    #distance_to_center = u.view(out_length, 1).expand(out_length, p) - indices
    distance_to_center = paddle.expand(paddle.reshape(u,[out_length, 1]),shape=[out_length, p])-indices
    # apply cubic kernel
    if (scale < 1) and antialiasing:
    ...
    weights_sum = paddle.reshape(paddle.sum(weights,1),[out_length, 1])
    #weights = weights / weights_sum.expand(out_length, p)
    weights = weights / paddle.expand(weights_sum,shape=[out_length, p])
...
def imresize(image: Any, scale_factor: float, antialiasing: bool = True) -> Any:
    ...
    if type(image).__module__ == np.__name__:  # numpy type
        ...
        if image.ndim == 2:
            image = image[:, :, None]
            squeeze_flag = True
        image = paddle.to_tensor(image.transpose(2, 0, 1),dtype='float32')
    else:
        numpy_type = False
        if image.ndim == 2:
    ...
    # process H dimension
    # symmetric copying
    img_aug = paddle.zeros(shape=[in_c, in_h + sym_len_hs + sym_len_he, in_w],dtype='float32')
    ...
    out_1 = paddle.zeros(shape=[in_c, out_h, in_w],dtype='float32')
    ...
    # process W dimension
    # symmetric copying
    out_1_aug = paddle.zeros(shape=[in_c, out_h, in_w + sym_len_ws + sym_len_we],dtype='float32')
    ...
    out_2 = paddle.zeros(shape=[in_c, out_h, out_w],dtype='float32')
